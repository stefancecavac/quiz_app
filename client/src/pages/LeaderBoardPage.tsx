import { useGetLeaderboards } from "../api/LeaderBoardApi";
import { TrophyIcon } from "../components/ui/icons/TrophyIcon";

export const LeaderBoardPage = () => {
  const { leaderBoardData, leaderBoardLoading } = useGetLeaderboards();

  return (
    <div className="grow h-full flex flex-col mx-20 ">
      <h2 className="text-4xl font-medium text-base-content">Top 100 users</h2>
      <div className="flex flex-col gap-3 mt-10 w-full ">
        {leaderBoardLoading ? (
          <span className="loading grow mx-auto justify-self-center loading-spinner text-primary" />
        ) : (
                ...
                <div className={`text-lg font-medium rounded-lg  flex items-center text-base-content  `}>
                  ...
                  {index !== 0 && index !== 1 && index !== 2 && (
                    <div className="size-10 flex items-center justify-center">
                      <p className="text-primary font-medium text-xl">{index + 1}</p>
                    </div>
                  )}
                </div>
                <p className="text-2xl font-medium text-base-content">{leaderBard.username}</p>
              </div>
              <div className="flex items-center gap-2">
                <div className="size-5">
                  <TrophyIcon />
                </div>
                <p className="font-medium text-yellow-400">{leaderBard.trophy}</p>
              </div>
            </div>
          ))
        )}
